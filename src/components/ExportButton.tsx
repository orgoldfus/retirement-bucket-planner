...
const ExportButton: React.FC<ExportButtonProps> = ({
  ...
  const handleExport = () => {
    ...
    csvContent += `Inflation Adjustment,${
      withdrawalPlan.adjustForInflation ? "Yes" : "No"
    }\n`;
    if (withdrawalPlan.adjustForInflation) {
      csvContent += `Inflation Rate,${withdrawalPlan.inflationRate}%\n`;
    }
    csvContent += `Time Frame,${
    ...
    csvContent += `Expected Portfolio Yield,${(
      calculatePortfolioYield(buckets, selectedTimeFrame) * 100
    ).toFixed(2)}%\n\n`;
    ...
    buckets.forEach((bucket) => {
      ...
      bucket.allocation.forEach((allocation) => {
        ...
        if (asset) {
          csvContent += `${asset.name},${allocation.percentage}%,${(
            asset.historicalReturn[selectedTimeFrame] * 100
          ).toFixed(2)}%\n`;
        }
      });

      csvContent += `Total Expected Return,${(
        calculateBucketYield(bucket, selectedTimeFrame) * 100
      ).toFixed(2)}%\n`;
    });
    ...
    projections.forEach((projection) => {
      csvContent += `${projection.year},${projection.portfolioValue.toFixed(
        2
      )},${projection.withdrawal.toFixed(2)},${projection.returns.toFixed(
        2
      )}\n`;
    });
