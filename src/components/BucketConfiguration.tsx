...
const BucketConfiguration: React.FC<BucketConfigurationProps> = ({ 
  ...
  const handleBucketChange = (index: number, field: keyof Bucket, value: any) => {
    const updatedBuckets = [...buckets];
    updatedBuckets[index] = { ...updatedBuckets[index], [field]: value };
    
    // Recalculate percentages if changing allocation
    if (field === 'percentage') {
      const newPercentage = Number(value) || 0;
      const oldPercentage = buckets[index].percentage;
      const difference = newPercentage - oldPercentage;
      
      // Distribute the difference among other buckets proportionally
      const otherBuckets = updatedBuckets.filter((_, i) => i !== index);
      const totalOtherPercentage = otherBuckets.reduce((sum, b) => sum + b.percentage, 0);
      
      if (totalOtherPercentage > 0) {
        otherBuckets.forEach(bucket => {
          const ratio = bucket.percentage / totalOtherPercentage;
          bucket.percentage = Math.max(0, bucket.percentage - (difference * ratio));
        });
      }
    }
  ...
  const removeBucket = (index: number) => {
    ...
    const removedPercentage = buckets[index].percentage;
    const updatedBuckets = buckets.filter((_, i) => i !== index);
    
    // Redistribute the removed bucket's percentage
    const remainingBuckets = updatedBuckets.length;
    const redistributedPercentage = removedPercentage / remainingBuckets;
    updatedBuckets.forEach(bucket => {
      bucket.percentage += redistributedPercentage;
    });
      ...
      <div className="space-y-6">
        <div className="bg-white border-3 border-black p-4 mb-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className={`font-brutal font-bold ${isInvalid ? 'text-error-500' : 'text-success-500'}`}>
              {totalPercentage.toFixed(1)}%
            </span>
          </div>
          <div className="w-full bg-neutral-200 border-3 border-black h-6">
            <div 
              className={`h-full ${isInvalid ? 'bg-error-500' : 'bg-success-500'}`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <NeoBrutalInput
                  ...
                  onChange={(e) => handleBucketChange(index, 'timeHorizon', Number(e.target.value) || 0)}
                  type="number"
                  min={1}
                  max={30}
                  required
              ...
              <div>
                <NeoBrutalInput
                  ...
                  onChange={(e) => handleBucketChange(index, 'percentage', Number(e.target.value) || 0)}
                  type="number"
                  min={0}
                  max={100}
                  required
        ...
        <div className="mt-4 p-4 bg-neutral-100 border-l-6 border-secondary-500">
          ...
          <p className="font-brutal text-sm">
            Time buckets are a strategy for organizing retirement savings based on when you'll need the money. Short-term buckets are for immediate needs (1-3 years), mid-term for intermediate needs (4-10 years), and long-term for distant needs (10+ years).
          </p>
        </div>
      </div>
    </NeoBrutalBox>
